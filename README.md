...
- 프론트엔드와 백엔드 간의 데이터 교환 형식을 사전에 명확히 정의하는 것이 얼마나 중요한지 깨달았습니다. 초기에 API 스펙을 상세히 문서화하지 않아 중간에 수정사항이 발생했을 때 양쪽 모두 코드를 수정해야 하는 상황이 자주 발생했습니다.

- 처음에는 각자 맡은 기능을 완성하면 전체 흐름이 자연스럽게 이어질 거라 생각했지만,
  복잡한 데이터 흐름을 다루면서 모든 팀원이 기능 간 연동을 지속적으로 공유하고 조율해야 한다는 것을 절실히 깨달았습니다.

- 브랜치 전략(`develop` → `main`)을 통한 협업에서 merge conflict를 해결하는 과정을 여러 번 경험했습니다. 특히 같은 파일을 동시에 수정할 때 발생하는 충돌을 해결하면서, 커밋 메시지 작성의 중요성과 정기적인 pull의 필요성을 느꼈습니다.

- 로컬에서는 정상 작동하던 기능이 배포 환경에서 오류가 발생하는 경우를 여러 번 겪었습니다. 환경변수 설정, CORS 정책, 파일 경로 등의 차이로 인한 문제들을 해결하면서 환경별 설정 관리의 중요성을 배웠습니다.

---

## 7. 전체 회고 및 느낀 점

- 이번 프로젝트를 통해 Node.js와 Express.js를 활용한 REST API 개발의 전체적인 흐름을 체험할 수 있었습니다. 단순히 라우터를 만드는 것을 넘어서 미들웨어, 인증, 데이터베이스 연동, 파일 처리까지 백엔드 개발의 핵심 요소들을 실제로 구현해보면서 웹 애플리케이션의 서버 사이드 로직에 대한 이해가 크게 향상되었습니다.

- 개발 과정에서 마주친 다양한 오류들을 해결하면서 문제를 체계적으로 분석하고 해결하는 능력이 향상되었습니다. 특히 에러 로그를 읽고 분석하는 능력, 공식 문서를 활용하는 방법, 그리고 동료와 협력하여 문제를 해결하는 과정에서 많은 것을 배웠습니다.
